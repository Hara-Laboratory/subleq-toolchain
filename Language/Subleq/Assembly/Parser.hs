...
parseExprParen = do
    ...
    args <- many (parseExpr <* spaces)
    -- e1 <- parseExpr
    -- e2 <- optionMaybe (space *> spaces *> parseExpr) <* spaces
    char ')'
    return $ op' op args
  where
    op' "+" [e1] = EAdd (Number 0) e1
    op' "+" [e1, e2'] = EAdd e1 e2'
    op' "-" [e1] = ESub (Number 0) e1
    op' "-" [e1, e2'] = ESub e1 e2'
    op' "shift" [e1, e2'] = EShiftL e1 e2'
...
parseExpr = try parseExprCurrentPos <|> parseExprParen <|> (Number <$> parseIntegerLiteral) <|> parseExternalReference <|> (Identifier <$> parseId)
